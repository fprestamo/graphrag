...
Etapa 2 (CGER) & \texttt{cger\_enabled}, \texttt{cger\_scorer}, pesos $w_1,\ldots,w_5$, \texttt{cger\_merge\_threshold}, \texttt{cger\_llm\_threshold\_low/high}, \texttt{cger\_candidate\_top\_k} \\
...
El módulo \texttt{entity\_resolution/scorers.py} implementa las cinco señales descritas en la sección~\ref{sec:bt-cger}, cada una expuesta como función pura para facilitar su prueba unitaria y su recombinación en distintos \emph{scorers}:

\begin{itemize}
  \item \textbf{Similitud coseno}: entre los vectores de descripción de las entidades.
  \item \textbf{BM25}: sobre los nombres canónicos.
  \item \textbf{Similitud de Jaccard}: a nivel de carácter.
  \item \textbf{Solapamiento temporal}: entre los intervalos $[\,\texttt{active\_start},\,\texttt{active\_end}\,]$.
  \item \textbf{Solapamiento de Jaccard}: entre los conjuntos de tipos de relación que cada entidad mantiene con sus vecinos en el grafo.
\end{itemize}

La función \texttt{compute\_entity\_composite\_score} agrega las cinco señales mediante una combinación lineal con pesos configurables. Antes de calcular el \emph{score}, un filtro previo de compatibilidad de tipos descarta, sin cómputo adicional, los pares manifiestamente incompatibles (por ejemplo, \texttt{PERSON} y \texttt{ORGANIZATION}). El parámetro \texttt{cger\_scorer} permite seleccionar variantes con un subconjunto de señales ---solo \emph{embedding}, o \emph{citation-and-description}---, lo que posibilita evaluar el aporte marginal de cada señal sin modificar el código.

CGER se implementa en \texttt{entity\_resolution/cger.py}, cuya función principal es \texttt{resolve\_entities}. El proceso se estructura en tres fases, siguiendo el diseño expuesto en la sección~\ref{sec:bt-cger}.
...
\paragraph{Fase 2 --- Scoring compuesto con política de tres umbrales.}
Sobre el subconjunto reducido se evalúa el \emph{score} compuesto aplicando la siguiente lógica:
...
\emph{score} $>$ \texttt{cger\_merge\_threshold} & Fusión automática \\
\emph{score} $<$ \texttt{cger\_llm\_threshold\_low} & Separación preservada \\
Zona de incertidumbre intermedia & Verificación por LLM \\ \hline
\end{tabular}
\caption{Política de tres umbrales para la decisión de fusión en CGER.}
\label{tab:impl-cger-thresholds}
\end{table}

\paragraph{Fase 3 --- Verificación por LLM.}
El \emph{prompt} de verificación recibe, para cada par en la zona de incertidumbre, el nombre, el tipo, la descripción, el período activo y el conjunto de relaciones conocidas de ambas entidades. El modelo devuelve una de tres etiquetas (\texttt{SAME}, \texttt{DIFFERENT}, \texttt{UNCERTAIN}) acompañada de una justificación.

Las decisiones de fusión se acumulan en un \texttt{merge\_map} que, una vez completada la pasada, se aplica también a las relaciones, garantizando la consistencia referencial entre nodos y aristas.

CGRR, implementado en \texttt{entity\_resolution/cgrr.py}, aplica el mismo esquema de tres fases que CGER, pero orientado a los tipos de relación. Su \emph{score} combina tres señales complementarias:
...
En la zona de incertidumbre el LLM es consultado siguiendo el mismo mecanismo que en CGER. El mapa de normalización resultante se propaga al campo \texttt{cardinality} de las aristas afectadas, garantizando que tanto la clasificación de cardinalidad como la detección de conflictos operen sobre un vocabulario canónico, libre de variantes léxicas.

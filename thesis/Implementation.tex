...
\begin{itemize}
  ...
  \item \texttt{entity\_resolution/}: resolución contextual de entidades (CGER) y el \emph{scorer} de similitud coseno sobre descripciones.
  ...
  \item \texttt{evaluation/}: infraestructura de evaluación, organizada por componente (CGER y ETCDR), con pruebas unitarias, validación sobre datos reales y búsqueda de hiperparámetros.
\end{itemize}
...
Etapa 3 (cardinalidad) & \texttt{relation\_cardinality\_overrides} \\
Etapa 4 (ETCDR) & \texttt{etcdr\_enabled}, \texttt{etcdr\_confidence\_threshold} \\
...
Cada subsistema dispone de un indicador de habilitación independiente (\texttt{cger\_enabled}, \texttt{etcdr\_enabled}), lo que aporta dos ventajas prácticas:
...
\begin{itemize}
  ...
  \item \textbf{Optimización dirigida por datos}: el umbral de activación del LLM en CGER (\texttt{cger\_cosine\_threshold}) y el umbral de confianza de ETCDR se exponen como parámetros independientes.
\end{itemize}
...
\section{Etapa 2: Resolución de entidades}\label{sec:impl-cger}

El módulo \texttt{entity\_resolution/scorers.py} expone como función pura la similitud coseno entre vectores de descripción, que es el único \emph{scorer} empleado por CGER. La función \texttt{description\_cosine\_entity\_scorer} recibe dos diccionarios y un objeto \texttt{BTGraphRAGConfig}, y devuelve el coseno entre los respectivos \emph{embeddings} de descripción. Al concentrar el \emph{scoring} en una sola señal, se elimina la necesidad de calibrar pesos heterogéneos y se traslada al LLM toda la responsabilidad sobre las decisiones cualitativas.
...
\section{Etapa 3: Clasificación de cardinalidad}\label{sec:impl-cardinality}
...
\section{Etapa 4: ETCDR}\label{sec:impl-etcdr}
...
\end{tabular}
\caption{Consultas Cypher sin restricción de tipo, que habilitan la detección de conflictos cruzados entre relaciones de distinta etiqueta pero la misma cardinalidad relevante.}
...
\section{Etapa 5: Almacén dual}\label{sec:impl-dual-store}
...
\begin{itemize}
  \item \textbf{Inicialización}: creación del controlador asíncrono y configuración del esquema (índices ordinarios sobre identificadores y \emph{labels}, e índices vectoriales para el prefiltrado de CGER).
...
\section{Etapa 6 (a): reportes de comunidad temporales y actualización incremental}\label{sec:impl-community}
...
\section{Etapa 6 (b): Consulta temporal}\label{sec:impl-query}
...
\begin{itemize}
  ...
  \item \textbf{Búsqueda de hiperparámetros}: optimización de los parámetros de CGER y ETCDR.
\end{itemize}
...
Similitud coseno de descripciones (CGER) & \texttt{entity\_resolution/scorers.py} \\
Resolución contextual de entidades & \texttt{entity\_resolution/cger.py} \\
Clasificación de cardinalidad y orquestación del \emph{pipeline} & \texttt{pipeline.py}, \texttt{prompts.py} \\

...
Etapa 2b (CGRR) & \texttt{cgrr\_enabled}, \texttt{cgrr\_cosine\_threshold}, \texttt{cgrr\_candidate\_top\_k} \\
...
\begin{itemize}
  ...
  \item \textbf{Optimización dirigida por datos}: los umbrales de activación del LLM en CGER y CGRR (\texttt{cger\_cosine\_threshold}, \texttt{cgrr\_cosine\_threshold}) y el umbral de confianza de ETCDR se exponen como parámetros independientes.
\end{itemize}
...
El módulo \texttt{entity\_resolution/scorers.py} expone como funciones puras la similitud coseno entre vectores de descripción, que es el único \emph{scorer} empleado tanto por CGER como por CGRR. Las dos funciones \texttt{description\_cosine\_entity\_scorer} y \texttt{description\_cosine\_relationship\_scorer} son simétricas: ambas reciben dos diccionarios y un objeto \texttt{BTGraphRAGConfig}, y devuelven el coseno entre los respectivos \emph{embeddings} de descripción. Al concentrar el \emph{scoring} en una sola señal, se elimina la necesidad de calibrar pesos heterogéneos y se traslada al LLM toda la responsabilidad sobre las decisiones cualitativas.
...
CGRR, implementado en \texttt{entity\_resolution/cgrr.py}, adopta el mismo esquema simplificado de CGER: un prefiltrado por similitud coseno sobre los \emph{embeddings} de descripción del tipo de relación, seguido de una verificación por LLM para los pares cuyo coseno supera el umbral \texttt{cgrr\_cosine\_threshold}. Por cada tipo de relación nuevo se recuperan los \texttt{cgrr\_candidate\_top\_k} tipos canónicos más similares mediante el índice vectorial sobre las relaciones de Neo4j (con respaldo en memoria si el índice no está disponible), filtrando además por solapamiento de entidades extremo: sólo se comparan tipos que comparten al menos un sujeto u objeto con el lote entrante. El mejor candidato $c^\ast$ por coseno $\sigma^\ast$ activa la regla de decisión:
...
\hline
\textbf{Condición} & \textbf{Acción} \\ \hline
$\sigma^\ast \geq$ \texttt{cgrr\_cosine\_threshold} & Verificación por LLM (\texttt{SAME} / \texttt{DIFFERENT}) \\
$\sigma^\ast <$ \texttt{cgrr\_cosine\_threshold} & Tipo preservado (sin llamada al LLM) \\ \hline
\end{tabular}
\caption{Regla de decisión de normalización en CGRR tras la simplificación del \emph{scoring}.}
\label{tab:impl-cgrr-thresholds}
\end{table}

El \emph{prompt} de verificación (constante \texttt{CGRR\_VERIFICATION\_PROMPT}) entrega, para cada uno de los dos tipos de relación, su nombre, su descripción y un par sujeto--objeto representativo. A diferencia de CGER, no incluye un período activo: los tipos de relación no portan una temporalidad propia, por lo que la única pregunta significativa es si los dos identificadores designan el mismo predicado. El modelo debe responder exactamente con uno de dos tokens:

\begin{itemize}
  \item \texttt{SAME}: los dos identificadores designan el mismo predicado (por ejemplo, \texttt{IS\_CEO\_OF} y \texttt{LEADS} entre persona y empresa; \texttt{WORKS\_FOR} y \texttt{EMPLOYED\_BY}). El candidato se normaliza a la forma canónica existente.
  \item \texttt{DIFFERENT}: los identificadores designan predicados distintos, ya sea por su alcance, su dirección o el tipo de entidades que conectan. El tipo candidato se conserva como entrada separada del vocabulario.
\end{itemize}

La función \texttt{llm\_verify\_relationship\_match} normaliza la salida del modelo y, ante cualquier token no reconocido, devuelve \texttt{DIFFERENT} como veredicto conservador por defecto. El mapa de normalización resultante se propaga al campo \texttt{cardinality} de las aristas afectadas, garantizando que tanto la clasificación de cardinalidad como la detección de conflictos operen sobre un vocabulario canónico, libre de variantes léxicas.
...
Similitud coseno de descripciones (CGER y CGRR) & \texttt{entity\_resolution/scorers.py} \\

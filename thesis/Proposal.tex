...
El \emph{pipeline} de indexación de BT-GRAPHRAG se organiza en seis etapas secuenciales. Las tres primeras procesan cada lote de documentos nuevos antes de modificar el grafo; las tres restantes persisten el grafo y mantienen las estructuras que se construyen a partir de él (la jerarquía de comunidades de Leiden y sus reportes), antes de dar paso a la fase de consulta.
...
% TODO: figura fig:bt-pipeline que muestre las seis etapas en secuencia, con sus entradas y salidas.
...
%% ============================================================
\section{Resolución contextual y temporal de entidades (CGER)}\label{sec:bt-cger}

CGER (\emph{Cross-Graph Entity Resolution}) es el módulo incorporado al \emph{pipeline} de GraphRAG para abordar la resolución de entidades desde una perspectiva contextual y temporal. Sustituye la coincidencia exacta de nombres del GraphRAG original por un procedimiento en dos fases: un \emph{scoring} basado en la similitud coseno entre los \emph{embeddings} de descripción de las entidades y, para los pares cuyo coseno supera un umbral, una verificación por LLM que razona conjuntamente sobre las descripciones y la temporalidad de cada entidad.
...
\begin{itemize}
    ...
    \item el módulo CGER de resolución contextual y temporal de entidades, y
    \item el módulo ETCDR de detección bidireccional de conflictos y resolución mediante \emph{Decision Router}.
\end{itemize}

Estos componentes se insertan en un \emph{pipeline} de seis etapas que preserva la compatibilidad con los modos de búsqueda local, global y DRIFT del sistema base, al tiempo que mantiene una representación bitemporal completa capaz de sostener las consultas \emph{as-of} y la detección bidireccional dirigida por cardinalidad.
...
1 & Temporalidad presente pero no integrada en el \emph{pipeline} de GraphRAG. & \emph{Pipeline} de seis etapas con la temporalidad como atributo de primera clase y representación dual del grafo (\ref{sec:bt-pipeline}). \\

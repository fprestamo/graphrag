...
\subsection{Resolución de tipos de relación}\label{subsec:bt-cgrr}

CGRR replica la arquitectura de dos fases de CGER sobre los identificadores de los tipos de relación, sustituyendo únicamente lo que la naturaleza del objeto resuelto obliga a sustituir. La primera fase es un prefiltrado por similitud coseno entre los \emph{embeddings} de descripción del tipo de relación: para cada tipo nuevo se recuperan los $K$ tipos canónicos más próximos mediante el índice vectorial sobre las aristas, filtrando además por solapamiento de entidades extremo, ya que sólo tiene sentido normalizar identificadores que aparecen entre los mismos sujetos u objetos. La segunda fase, activada cuando el coseno del mejor candidato supera el umbral $\tau_{\cos}^{\,r}$, consulta al LLM con el nombre del tipo, su descripción y un par sujeto--objeto representativo de cada lado, y le solicita exactamente una de dos etiquetas:

\begin{itemize}
    \item \texttt{SAME}: los dos identificadores designan el mismo predicado (por ejemplo, \texttt{IS\_CEO\_OF} y \texttt{LEADS} entre persona y empresa, o \texttt{WORKS\_FOR} y \texttt{EMPLOYED\_BY}). El tipo candidato se normaliza al canónico.

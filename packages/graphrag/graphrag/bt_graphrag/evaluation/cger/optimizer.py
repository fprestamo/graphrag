"""Metaheuristic hyperparameter optimizer for CGER scorers.

Exposes ``optimize(scorer_name, …)`` which runs DE, SA, and/or PSO and
returns the best parameters per scorer as an ``OptimizationResult``.
"""
...
SEARCH_SPACES: dict[str, list[ParamBound]] = {
    "composite_5signal": [
        ParamBound("embedding_weight",        0.05, 0.80, "cger_embedding_weight"),
        ParamBound("bm25_weight",             0.05, 0.60, "cger_bm25_weight"),
        ParamBound("jaccard_weight",          0.01, 0.40, "cger_jaccard_weight"),
        ParamBound("temporal_overlap_weight", 0.00, 0.30, "cger_temporal_overlap_weight"),
        ParamBound("relation_context_weight", 0.00, 0.30, "cger_relation_context_weight"),
        ParamBound("merge_threshold",         0.40, 0.95, "cger_merge_threshold"),
        ParamBound("llm_threshold_low",       0.20, 0.80, "cger_llm_threshold_low"),
    ],
    "embedding_only": [
        ParamBound("merge_threshold",   0.40, 0.99, "cger_merge_threshold"),
        ParamBound("llm_threshold_low", 0.20, 0.85, "cger_llm_threshold_low"),
    ],
...
def normalize_weights(cfg: BTGraphRAGConfig) -> BTGraphRAGConfig:
    total = (cfg.cger_embedding_weight + cfg.cger_bm25_weight + cfg.cger_jaccard_weight
             + cfg.cger_temporal_overlap_weight + cfg.cger_relation_context_weight)
    if total > 0:
        cfg.cger_embedding_weight        /= total
        cfg.cger_bm25_weight             /= total
        cfg.cger_jaccard_weight          /= total
        cfg.cger_temporal_overlap_weight /= total
        cfg.cger_relation_context_weight /= total
    return cfg
...
def _objective(vec, scorer_name, scorer_fn, entities, merges, space):
    from graphrag.bt_graphrag.evaluation.cger.harness import evaluate_scorer
    cfg = decode_params(vec, space)
    if scorer_name == "composite_5signal":
        cfg = normalize_weights(cfg)
    return -evaluate_scorer(scorer_fn, scorer_name, entities, merges, cfg).f1


# ---------------------------------------------------------------------------
# DE / SA / PSO  (same algorithms, generalised)
# ---------------------------------------------------------------------------

def _de(obj, ndim, pop=30, iters=80, F=0.8, CR=0.9, seed=42):
    rng = random.Random(seed)
    pop_ = [[rng.random() for _ in range(ndim)] for _ in range(pop)]
    fit = [obj(ind) for ind in pop_]
    hist = []
    for gen in range(1, iters + 1):
        for i in range(pop):
            idxs = [x for x in range(pop) if x != i]
            a, b, c = rng.sample(idxs, 3)
            j0 = rng.randint(0, ndim - 1)
            trial = [max(0., min(1., pop_[a][d] + F * (pop_[b][d] - pop_[c][d]))
                         if (rng.random() < CR or d == j0) else pop_[i][d])
                     for d in range(ndim)]
            ft = obj(trial)
            if ft <= fit[i]:
                pop_[i], fit[i] = trial, ft
        if gen % 10 == 0:
...
async def optimize(
    ...
    import json
    from graphrag.bt_graphrag.evaluation.cger.harness import build_entity_dicts
    ...
    if ground_truth is None:
        if ground_truth_path:
            ...
            from graphrag.bt_graphrag.evaluation.cger.ground_truth import build_ground_truth
            ground_truth = build_ground_truth()

    entities = await build_entity_dicts(ground_truth, embedding_fn)
    space = SEARCH_SPACES[scorer_name]
    scorer_fn = SCORER_REGISTRY[scorer_name]
    ndim = len(space)
    merges = ground_truth["expected_merges"]

    def obj(v): return _objective(v, scorer_name, scorer_fn, entities, merges, space)

    results: list[OptimizationResult] = []
    for method in (methods or ["de", "sa", "pso"]):
        print(f"\n    [CGER Optimizer] {method.upper()} for '{scorer_name}' ({ndim} dims)…")
        t0 = time.time()
        if method == "de":
        ...
        # Add normalised weight breakdown for composite
        if scorer_name == "composite_5signal":
            tw = sum(params[b.name] for b in space[:5])
            if tw > 0:
                for b in space[:5]:
                    params[f"{b.name}_norm"] = round(params[b.name] / tw, 4)

        results.append(OptimizationResult(
            scorer_name=scorer_name, component="CGER",
            method=method, best_params=params,
            best_f1=bf, elapsed_seconds=elapsed, history=hist,
        ))
        print(f"    [CGER Optimizer] {method.upper()} done → F1={bf:.4f} in {elapsed:.1f}s")
    return results

...
\begin{itemize}
    ...
    \item \textbf{Resolución temporal de entidades.} Para determinar si dos menciones se refieren a la misma entidad, el sistema incorpora el solapamiento de períodos activos como señal adicional en la función de \emph{scoring}. Este mecanismo evita tanto la fusión inadecuada de estados muy distintos de una misma entidad como la fragmentación errónea de una entidad cuyo nombre haya cambiado a lo largo del tiempo.
...
\begin{enumerate}
    ...
    \item \textbf{Resolución contextual de entidades.} Las entidades extraídas se contrastan con las ya presentes en el grafo mediante un proceso en tres fases: un prefiltrado por similitud, una función de \emph{scoring} compuesta y, en los casos ambiguos, una verificación por LLM que incorpora el período activo de cada entidad como parte del contexto. El procedimiento completo se detalla en la sección~\ref{sec:bt-cger}.
...
CGER y CGRR son dos módulos incorporados al \emph{pipeline} de GraphRAG con el propósito de abordar la resolución de entidades y de tipos de relación desde una perspectiva contextual y temporal. CGER (\emph{Cross-Graph Entity Resolution}) sustituye la coincidencia exacta de nombres del GraphRAG original por un procedimiento en tres fases: pre-filtrado por similitud, \emph{scoring} compuesto y verificación por LLM en la zona de incertidumbre, las cuales se explican a continuación. CGRR (\emph{Cross-Graph Relationship Resolution}) aplica el mismo esquema a los tipos de relación.

\subsection{Pre-filtrado por similitud}

La comparación exhaustiva de cada entidad nueva contra la totalidad de entidades presentes en el grafo resulta computacionalmente costosa, en cuanto a su complejidad temporal, en corpus de gran tamaño. Para acotar el espacio de búsqueda, CGER realiza una consulta por similitud sobre los \emph{embeddings} de descripción de las entidades: dado un candidato, se recuperan sus $K$ vecinos más próximos según la similitud coseno~\cite{TODO_cosine_similarity}. El parámetro $K$ es configurable en función del dominio y del tamaño del corpus. La evaluación del \emph{scoring} completo se restringe exclusivamente a este subconjunto, lo que transforma una operación de complejidad cuadrática en una operación lineal respecto al número de entidades nuevas.

\subsection{Scoring compuesto de cinco señales}

Para cada par (entidad nueva, candidato del grafo), CGER calcula un \emph{score} agregado a partir de cinco señales independientes:

\begin{enumerate}
    \item \textbf{Similitud semántica de descripción.} Coseno entre los \emph{embeddings} de las descripciones generadas durante la extracción.
    \item \textbf{BM25 sobre nombres}~\cite{TODO_bm25}. Similitud léxica clásica entre los nombres canónicos de las dos entidades.
    \item \textbf{Jaccard de caracteres}~\cite{TODO_jaccard}. Similitud morfológica que captura variaciones tipográficas, abreviaturas y errores menores que BM25 no detecta.
    \item \textbf{Solapamiento de períodos activos.} Cociente entre la intersección y la unión de los intervalos $[\,\texttt{active\_start}, \texttt{active\_end}\,]$ de las dos entidades.
    \item \textbf{Similitud del contexto relacional.} Solapamiento de Jaccard entre los conjuntos de tipos de relación que cada entidad mantiene con sus vecinos en el grafo.
\end{enumerate}

La puntuación final se obtiene como combinación lineal de las cinco señales antes mencionadas, con pesos configurables. La cuarta señal constituye una aportación propia de BT-GRAPHRAG. Su utilidad queda ilustrada en el ejemplo ``Apple 1985 vs.\ 2024'', introducido en la sección~\ref{subsec:entity-resolution}. En dicho ejemplo, las descripciones, los \emph{embeddings} e incluso la similitud léxica sobre el nombre coinciden de forma casi perfecta entre ambas menciones. En ausencia de información temporal, sistemas como DALK~\cite{dalk2024} y DYNAMO~\cite{dynamo2025} fusionarían ambas entradas en un único nodo, perdiendo así la diferencia de estado entre los dos períodos. La señal de solapamiento temporal introduce una penalización suficiente cuando los intervalos activos de dos menciones están separados por décadas, de modo que la puntuación agregada queda por debajo del umbral de fusión y ambas entidades se preservan como nodos diferenciados.

% TODO: figura fig:bt-cger-pipeline que represente las tres fases de CGER como un pipeline horizontal: (1) prefiltrado por similitud vectorial (top-K), (2) scoring compuesto de cinco señales con sus pesos, (3) política de tres umbrales con la zona de incertidumbre que dispara la verificación por LLM; las cinco señales (semántica, BM25, Jaccard, solapamiento temporal, contexto relacional) deben aparecer como entradas claramente etiquetadas a la fase 2.
...
El doble umbral acota el costo asociado a las llamadas al LLM. El hecho de que el modelo reciba el período activo como parte del contexto le permite, además, replicar en el plano cualitativo la decisión que la señal de solapamiento temporal ya orientó cuantitativamente.

\subsection{Resolución de tipos de relación}

CGRR aplica el mismo esquema a los tipos de relación. El módulo recibe los tipos extraídos por el LLM y los contrasta con los ya consolidados en el grafo. La función de \emph{scoring} combina BM25 sobre el nombre del tipo, similitud semántica de la descripción y un indicador de coincidencia de extremos: cuando los pares sujeto-objeto se solapan, la evidencia de equivalencia semántica se refuerza. Así, por ejemplo, el sistema puede fusionar \texttt{IS\_CEO\_OF} y \texttt{LEADS} cuando el contexto indica que ambas etiquetas designan el mismo predicado. Este proceso de consolidación reduce la dispersión del vocabulario de relaciones antes de invocar al módulo de clasificación de cardinalidad y al de detección de conflictos.

En esta sección se da respuesta a la limitación~5 identificada en el estado del arte. La resolución de entidades en sistemas como DALK y DYNAMO es de naturaleza puramente semántica y, como se señala en la sección~\ref{subsec:entity-resolution}, no contempla la posibilidad de que dos menciones pertenecientes a períodos distintos describan estados sustancialmente diferentes de una misma entidad. CGER aborda esta carencia incorporando el solapamiento temporal como señal explícita en la función de \emph{scoring} y habilitando al LLM para que razone sobre el período activo como parte del contexto de decisión. Por su parte, la consolidación de tipos que aporta CGRR garantiza que tanto la clasificación de cardinalidad como la detección de conflictos operen sobre un vocabulario canónico y sin ambigüedades léxicas.
